...
public class Entity {
	...
	public void move(float xa, float ya, float za) {
		...
		for(int i = 0; i < aABBs.size(); ++i) ya = ((AABB)aABBs.get(i)).clipYCollide(this.bb, ya);
		this.bb.move(0.0F, ya, 0.0F);

		for(int i = 0; i < aABBs.size(); ++i) xa = ((AABB)aABBs.get(i)).clipXCollide(this.bb, xa);
		this.bb.move(xa, 0.0F, 0.0F);

		for(int i = 0; i < aABBs.size(); ++i) za = ((AABB)aABBs.get(i)).clipZCollide(this.bb, za);
